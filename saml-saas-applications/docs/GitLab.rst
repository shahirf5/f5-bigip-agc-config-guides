...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure GitLab:
...
GitLab Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure GitLab, select GitLab and click :guilabel:`Add`.
...
To configure GitLab provide following inputs:
	- :guilabel:`Domain Name` : Provide the domain name from the callback URL. For eg - If the callback URL is https://company.com/users/auth/saml/callback, then domain name is company.com.
...
#. Select the SAML SSO object created for for GitLab, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the GitLab service.

Setup GitLab as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to GitLab documentation to set it up as SAML Service Provider.
