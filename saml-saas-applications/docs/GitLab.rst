...
For example, to configure GitLab, select GitLab and click **Add**.
...
To configure GitLab provide following inputs:
	- **Domain Name** : Provide the domain name from the callback URL. For eg - If the callback URL is https://company.com/users/auth/saml/callback, then domain name is company.com.
...
#. Select the SAML SSO object created for for GitLab, and click **Export Metadata**.
