...
For example, to configure Syncplicity, select Syncplicity and click ``Add``.
...
To configure Syncplicity provide following inputs:
	- ``Sub Domain`` : Provide the sub domain. For eg - If the SSO URL is https://company.syncplicity.com/Auth/AssertionConsumerService.aspx, then the sub domain is company.
...
#. Select the SAML SSO object created for for Syncplicity, and click ``Export Metadata``.
