...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Planview:
...
Planview Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Planview, select Planview and click :guilabel:`Add`.
...
To configure Planview provide following inputs:
	- :guilabel:`Sub Domain` : Specify sub domain. If Entity ID is https://acme.pvcloud.com/planview/, enter acme
...
Refer to Planview documentation to set it up as SAML Service Provider.
