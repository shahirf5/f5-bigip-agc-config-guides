...
--------------------------------
This document describes configuration steps for configuring an AGC SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Mimecast (Admin):
...
Mimecast (Admin) Configuration in AGC Workflow
----------------------------------------------
...
Mimecast (Admin), select
Mimecast (Admin) and click on **Add** button
...
#. Identify the Saml SSO object created for for Mimecast (Admin) and export SAML Metadata.

You can use the exported IdP SAML Metadata to configure the IdP Provider configuration in Mimecast (Admin) service.
