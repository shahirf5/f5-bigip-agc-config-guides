...
--------------------------------
This document describes configuration steps for configuring an AGC SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Panopto:
...
Panopto Configuration in AGC Workflow
---------------------------------------------------------------------------
...
Panopto, select
Panopto and click on **Add** button
...
To configure Panopto provide following inputs:
	- **Panopto Server Name** : Specify Panopto Server Name which is used to setup Panopto SP entityId. For example, if your ACS URL is https://serverName/panopto. then enter serverName.
...
#. Identify the Saml SSO object created for for Panopto and export SAML Metadata.

You can use the exported IdP SAML Metadata to configure the IdP Provider configuration in Panopto service.
