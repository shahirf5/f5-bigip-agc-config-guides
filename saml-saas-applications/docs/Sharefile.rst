...
For example, to configure Sharefile, select Sharefile and click ``Add``.
...
To configure Sharefile provide following inputs:
	- ``Sub Domain`` : Provide your sub domain. If the SSO URL is like https://company.sharefile.com/saml/acs, then sub domain is company
...
#. Select the SAML SSO object created for for Sharefile, and click ``Export Metadata``.
