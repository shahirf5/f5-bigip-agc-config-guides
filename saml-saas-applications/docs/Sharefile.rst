...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Sharefile:
...
Sharefile Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Sharefile, select Sharefile and click :guilabel:`Add`.
...
To configure Sharefile provide following inputs:
	- :guilabel:`Sub Domain` : Provide your sub domain. If the SSO URL is like https://company.sharefile.com/saml/acs, then sub domain is company
...
#. Select the SAML SSO object created for for Sharefile, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Sharefile service.

Setup Sharefile as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Sharefile documentation to set it up as SAML Service Provider.
