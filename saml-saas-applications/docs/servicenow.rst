...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure ServiceNow:
...
ServiceNow Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure ServiceNow, select ServiceNow and click :guilabel:`Add`.
...
To configure ServiceNow provide following inputs:
	- :guilabel:`Sub Domain` : Provide your sub domain from the SSO URL. For eg - If the URL is https://company.service-now.com, then sub domain is company.
...
#. Select the SAML SSO object created for for ServiceNow, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the ServiceNow service.

Setup ServiceNow as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to ServiceNow documentation to set it up as SAML Service Provider.
