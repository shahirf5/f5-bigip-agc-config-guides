...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure securitymentor:
...
securitymentor Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure securitymentor, select securitymentor and click :guilabel:`Add`.
...
To configure securitymentor provide following inputs:
	- :guilabel:`Organization ID` : Provide the organization ID. For eg - If ACS URL is https://www.securitymentor.com/simplesaml/module.php/saml/sp/saml2-acs.php/company, then orgId is company.
...
#. Select the SAML SSO object created for for securitymentor, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the securitymentor service.

Setup securitymentor as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to securitymentor documentation to set it up as SAML Service Provider.
