...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure DocuSign:
...
DocuSign Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure DocuSign, select DocuSign and click :guilabel:`Add`.
...
To configure DocuSign provide following inputs:
	- :guilabel:`Account Type` : Select the type of docusign account you have.
	- :guilabel:`Organzation ID` : Provide the organization id registered with docusign. For eg - If the ACS URL is https://account.docusign.com/organizations/abcd1234/saml2, then abcd1234 is the organization id.
...
#. Select the SAML SSO object created for for DocuSign, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the DocuSign service.
