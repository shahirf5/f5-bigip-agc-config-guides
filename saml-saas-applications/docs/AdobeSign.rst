...
For example, to configure Adobe Sign (formerly Echosign), select Adobe Sign (formerly Echosign) and click :guilabel:`Add`.
...
To configure Adobe Sign (formerly Echosign) provide following inputs:
	- :guilabel:`Hostname` : Provide the hostname configured in Adobe Sign. You can obtain this from the ACS URL. For eg - If ACS URL is https://company.na1.echosign.com/public/samlConsume then the hostname is company.
...
#. Select the SAML SSO object created for for Adobe Sign (formerly Echosign), and click :guilabel:`Export Metadata`.
...
Refer to Adobe Sign (formerly Echosign) documentation to set it up SAML Service Provider.
