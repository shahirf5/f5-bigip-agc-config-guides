...
For example, to configure Bioraft, select Bioraft and click :guilabel:`Add`.
...
To configure Bioraft provide following inputs:
	- :guilabel:`Sub Domain` : Provide the sub domain. For eg - If the SSO URL is https://company.bioraft.com/Shibboleth.sso/SAML2/POST, then the sub domain is company.
...
#. Select the SAML SSO object created for for Bioraft, and click :guilabel:`Export Metadata`.
