...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Allocadia:
...
Allocadia Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Allocadia, select Allocadia and click :guilabel:`Add`.
...
To configure Allocadia provide following inputs:
	- :guilabel:`Sub Domain` : Provide the sub domain name from the SSO URL. For eg - If the URL is https://company.allocadia.com/allocadia/saml/SSO, then the sub domain is company.
...
#. Select the SAML SSO object created for for Allocadia, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Allocadia service.

Setup Allocadia as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Allocadia documentation to set it up as SAML Service Provider.
