...
For example, to configure Allocadia, select Allocadia and click ``Add``.
...
To configure Allocadia provide following inputs:
	- ``Sub Domain`` : Provide the sub domain name from the SSO URL. For eg - If the URL is https://company.allocadia.com/allocadia/saml/SSO, then the sub domain is company.
...
#. Select the SAML SSO object created for for Allocadia, and click ``Export Metadata``.
