...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Planisware:
...
Planisware Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Planisware, select Planisware and click :guilabel:`Add`.
...
To configure Planisware provide following inputs:
	- :guilabel:`Sub Domain` : Specify sub domain. If your ACS URI is https://acme.planisware.com:443/sp/ACS.saml2, then enter acme for sub domain.
...
#. Select the SAML SSO object created for for Planisware, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Planisware service.

Setup Planisware as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Planisware documentation to set it up as SAML Service Provider.
