...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Jive:
...
Jive Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Jive, select Jive and click :guilabel:`Add`.
...
To configure Jive provide following inputs:
	- :guilabel:`Sub Domain` : Specify Sub Domain value. For example, if the entity ID URL is https://company.jiveon.com, then enter company
...
Refer to Jive documentation to set it up as SAML Service Provider.
