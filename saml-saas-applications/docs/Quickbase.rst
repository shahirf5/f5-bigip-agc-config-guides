...
For example, to configure Quickbase, select Quickbase and click ``Add``.
...
To configure Quickbase provide following inputs:
	- ``Sub Domain`` : Provide the sub domain from the SSO URL. For eg - If the SSO URL is https://company.quickbase.com, then sub domain is company.
...
#. Select the SAML SSO object created for for Quickbase, and click ``Export Metadata``.
