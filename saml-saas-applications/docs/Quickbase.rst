...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Quickbase:
...
Quickbase Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Quickbase, select Quickbase and click :guilabel:`Add`.
...
To configure Quickbase provide following inputs:
	- :guilabel:`Sub Domain` : Provide the sub domain from the SSO URL. For eg - If the SSO URL is https://company.quickbase.com, then sub domain is company.
...
#. Select the SAML SSO object created for for Quickbase, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Quickbase service.

Setup Quickbase as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Quickbase documentation to set it up as SAML Service Provider.
