...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure InspiredLMS:
...
InspiredLMS Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure InspiredLMS, select InspiredLMS and click :guilabel:`Add`.
...
To configure InspiredLMS provide following inputs:
	- :guilabel:`IdP Issuer Identifier` : Please provide the unique issuer identifier provisioned by InspiredLMS.
	- :guilabel:`Company Name` : Provide the company name. For ex, if your ACS URL is https://www.inspiredlms.com/Login/company/consumer.aspx, then enter company
...
#. Select the SAML SSO object created for for InspiredLMS, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the InspiredLMS service.

Setup InspiredLMS as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to InspiredLMS documentation to set it up as SAML Service Provider.
