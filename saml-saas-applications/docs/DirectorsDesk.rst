...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure DirectorsDesk:
...
DirectorsDesk Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure DirectorsDesk, select DirectorsDesk and click :guilabel:`Add`.
...
To configure DirectorsDesk provide following inputs:
	- :guilabel:`Sub Domain` : Provide the sub domain. For eg - If the ACS URL is https://company.directorsdesk.com/saml2login.aspx, then the sub domain is company.
...
#. Select the SAML SSO object created for for DirectorsDesk, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the DirectorsDesk service.

Setup DirectorsDesk as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to DirectorsDesk documentation to set it up as SAML Service Provider.
