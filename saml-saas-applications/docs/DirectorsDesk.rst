...
For example, to configure DirectorsDesk, select DirectorsDesk and click ``Add``.
...
To configure DirectorsDesk provide following inputs:
	- ``Sub Domain`` : Provide the sub domain. For eg - If the ACS URL is https://company.directorsdesk.com/saml2login.aspx, then the sub domain is company.
...
#. Select the SAML SSO object created for for DirectorsDesk, and click ``Export Metadata``.
