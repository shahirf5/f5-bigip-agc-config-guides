...
For example, to configure Slack, select Slack and click ``Add``.
...
To configure Slack provide following inputs:
	- ``Sub Domain`` : Provide the sub domain from SSO URL. For eg - If URL is https://company.slack.com/sso/saml, then company is the sub domain.
...
#. Select the SAML SSO object created for for Slack, and click ``Export Metadata``.
