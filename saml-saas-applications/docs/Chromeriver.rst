...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Chromeriver:
...
Chromeriver Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Chromeriver, select Chromeriver and click :guilabel:`Add`.
...
To configure Chromeriver provide following inputs:
	- :guilabel:`Server Type` : Select if Production or QA server
	- :guilabel:`Customer ID` : Specify Customer ID value. SSO URI uses this value. For example - If SSO URI is https://qa-app.chromeriver.com/login/sso/saml/consume\?customerId=xyz, customer ID is xyz
...
Refer to Chromeriver documentation to set it up as SAML Service Provider.
