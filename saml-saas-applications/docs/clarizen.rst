...
For example, to configure Clarizen, select Clarizen and click ``Add``.
...
To configure Clarizen provide following inputs:
	- ``Sub Domain`` : Provide the sub domain. For eg if the ACS URL is https://company.clarizen.com/Clarizen/Pages/Integrations/SAML/SamlResponse.aspx, the sub domain is company.
...
#. Select the SAML SSO object created for for Clarizen, and click ``Export Metadata``.
