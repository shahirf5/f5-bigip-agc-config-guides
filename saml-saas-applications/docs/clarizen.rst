...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Clarizen:
...
Clarizen Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Clarizen, select Clarizen and click :guilabel:`Add`.
...
To configure Clarizen provide following inputs:
	- :guilabel:`Sub Domain` : Provide the sub domain. For eg if the ACS URL is https://company.clarizen.com/Clarizen/Pages/Integrations/SAML/SamlResponse.aspx, the sub domain is company.
...
#. Select the SAML SSO object created for for Clarizen, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Clarizen service.

Setup Clarizen as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Clarizen documentation to set it up as SAML Service Provider.
