...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Replicon:
...
Replicon Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Replicon, select Replicon and click :guilabel:`Add`.
...
To configure Replicon provide following inputs:
	- :guilabel:`Company Key` : Specify a Company Key value. Entity ID and ACS URL uses this value. If your ACS URL is: https://global.replicon.com/!/saml2/companyKey, then enter companyKey
...
#. Select the SAML SSO object created for for Replicon, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Replicon service.
