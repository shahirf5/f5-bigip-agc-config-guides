...
For example, to configure Replicon, select Replicon and click ``Add``.
...
To configure Replicon provide following inputs:
	- ``Company Key`` : Specify a Company Key value. Entity ID and ACS URL uses this value. If your ACS URL is: https://global.replicon.com/!/saml2/companyKey, then enter companyKey
...
#. Select the SAML SSO object created for for Replicon, and click ``Export Metadata``.
