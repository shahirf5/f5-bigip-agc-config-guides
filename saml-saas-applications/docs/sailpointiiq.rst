...
For example, to configure Sailpoint IIQ, select Sailpoint IIQ and click ``Add``.
...
To configure Sailpoint IIQ provide following inputs:
	- ``Sailpoint IIQ host`` : Specify Sailpoint IIQ host. Entity ID uses this value. If ACS URL is https://company/identityiq/login.jsf, then Sailpoint IIQ host is company
...
#. Select the SAML SSO object created for for Sailpoint IIQ, and click ``Export Metadata``.
