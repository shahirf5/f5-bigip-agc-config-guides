...
For example, to configure Clicksoftware, select Clicksoftware and click ``Add``.
...
To configure Clicksoftware provide following inputs:
	- ``Sub Domain`` : Provide the sub domain. For eg - If the SSO URL is https://company.clicksoftware.com/cgi/samlauth, then the sub domain is company.
...
#. Select the SAML SSO object created for for Clicksoftware, and click ``Export Metadata``.
