...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Zendesk:
...
Zendesk Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Zendesk, select Zendesk and click :guilabel:`Add`.
...
To configure Zendesk provide following inputs:
	- :guilabel:`Sub Domain` : Provide your sub domain from the SSO URL. For eg- If SSO URL is https://company.zendesk.com/access/saml, then the sub domain is company.
...
#. Select the SAML SSO object created for for Zendesk, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Zendesk service.

Setup Zendesk as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Zendesk documentation to set it up as SAML Service Provider.
