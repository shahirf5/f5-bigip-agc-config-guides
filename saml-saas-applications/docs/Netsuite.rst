...
For example, to configure Netsuite, select Netsuite and click ``Add``.
...
To configure Netsuite provide following inputs:
	...
	- ``Sub Domain`` : Provide the sub domain from the SSO URL. For eg - If URL is https://system.testcompany.netsuite.com/saml2/acs, then sub domain is testcompany.
...
#. Select the SAML SSO object created for for Netsuite, and click ``Export Metadata``.
