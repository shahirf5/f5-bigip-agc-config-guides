...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Jenkins:
...
Jenkins Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Jenkins, select Jenkins and click :guilabel:`Add`.
...
To configure Jenkins provide following inputs:
	- :guilabel:`Hostname` : Enter your Hostname. For example, if your ACS URL is https://yourcompany.com/securityRealm/finishLogin, enter: yourcompany.com
...
#. Select the SAML SSO object created for for Jenkins, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Jenkins service.

Setup Jenkins as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Jenkins documentation to set it up as SAML Service Provider.
