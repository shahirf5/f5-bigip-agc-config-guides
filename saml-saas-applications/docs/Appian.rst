...
For example, to configure Appian, select Appian and click ``Add``.
...
To configure Appian provide following inputs:
	- ``Sub Domain`` : Provide the sub domain. For eg - If the SSO URL is https://company.appiancloud.com/suite/saml/AssertionConsumer, then the sub domain is company.
...
#. Select the SAML SSO object created for for Appian, and click ``Export Metadata``.
