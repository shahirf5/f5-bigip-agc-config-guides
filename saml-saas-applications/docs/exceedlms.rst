...
For example, to configure ExceedLMS (Intellum), select ExceedLMS (Intellum) and click ``Add``.
...
To configure ExceedLMS (Intellum) provide following inputs:
	...
	- ``Sub Domain`` : Enter your sub domain. For example, if your ACS URL is https://yourcompany.exceedlms.com/authentication/saml_login, enter: yourcompany
...
#. Select the SAML SSO object created for for ExceedLMS (Intellum), and click ``Export Metadata``.
