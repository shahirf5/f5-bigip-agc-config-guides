...
For example, to configure CMO Compliance, select CMO Compliance and click ``Add``.
...
To configure CMO Compliance provide following inputs:
	- ``Sub Domain`` : Provide the Sub Domain. For e.g. if the Entity Id URL is http://company.cmo-compliance.com/adfs/services/trust, the Sub Domain is company
...
#. Select the SAML SSO object created for for CMO Compliance, and click ``Export Metadata``.
