...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Splunk:
...
Splunk Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Splunk, select Splunk and click :guilabel:`Add`.
...
To configure Splunk provide following inputs:
	- :guilabel:`Splunk Entity ID` : Provide the entity ID from Splunk configuration.
	- :guilabel:`Splunk Hostname` : Provide the hostname from the SSO URL. For eg, If the SSO URL is https://abcd.com/saml/acs, then the hostname is abcd.com.
...
#. Select the SAML SSO object created for for Splunk, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Splunk service.

Setup Splunk as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Splunk documentation to set it up as SAML Service Provider.
