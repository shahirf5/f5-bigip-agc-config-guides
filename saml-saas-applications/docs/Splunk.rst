...
For example, to configure Splunk, select Splunk and click ``Add``.
...
To configure Splunk provide following inputs:
	- ``Splunk Entity ID`` : Provide the entity ID from Splunk configuration.
	- ``Splunk Hostname`` : Provide the hostname from the SSO URL. For eg, If the SSO URL is https://abcd.com/saml/acs, then the hostname is abcd.com.
...
#. Select the SAML SSO object created for for Splunk, and click ``Export Metadata``.
