...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Sprinklr:
...
Sprinklr Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Sprinklr, select Sprinklr and click :guilabel:`Add`.
...
To configure Sprinklr provide following inputs:
	- :guilabel:`Sub Domain` : Provide the sub domain. For eg - In SSO URL, https://company.sprinklr.com/ui/sso/saml/249, company is the sub domain
	- :guilabel:`Account Number` : In SSO URL https://company.sprinklr.com/ui/sso/saml/135, 135 is the account number
...
#. Select the SAML SSO object created for for Sprinklr, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Sprinklr service.

Setup Sprinklr as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Sprinklr documentation to set it up as SAML Service Provider.
