...
For example, to configure Desk, select Desk and click ``Add``.
...
To configure Desk provide following inputs:
	- ``Sub Domain`` : Provide the sub domain. For eg - If the URL is https://company.desk.com/auth/saml/acs, then the sub domain is company.
...
#. Select the SAML SSO object created for for Desk, and click ``Export Metadata``.
