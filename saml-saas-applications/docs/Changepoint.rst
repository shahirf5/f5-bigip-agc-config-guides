...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Changepoint:
...
Changepoint Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Changepoint, select Changepoint and click :guilabel:`Add`.
...
To configure Changepoint provide following inputs:
	- :guilabel:`Hostname` : Enter Hostname. For example, if your ACS URL is https://changepoint.com/RP-STS_SAML/SAML/AssertionConsumerService.aspx, enter: changepoint.com
...
#. Select the SAML SSO object created for for Changepoint, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Changepoint service.

Setup Changepoint as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Changepoint documentation to set it up as SAML Service Provider.
