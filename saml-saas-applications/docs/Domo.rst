...
For example, to configure Domo, select Domo and click ``Add``.
...
To configure Domo provide following inputs:
	- ``Company Sub Domain`` : Provide the company sub domain from the ACS URL For eg - In ACS URL https://company.domo.com/auth/saml, company is the sub domain.
...
#. Select the SAML SSO object created for for Domo, and click ``Export Metadata``.
