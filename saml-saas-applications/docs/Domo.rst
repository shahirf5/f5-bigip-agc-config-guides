...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Domo:
...
Domo Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Domo, select Domo and click :guilabel:`Add`.
...
To configure Domo provide following inputs:
	- :guilabel:`Company Sub Domain` : Provide the company sub domain from the ACS URL For eg - In ACS URL https://company.domo.com/auth/saml, company is the sub domain.
...
#. Select the SAML SSO object created for for Domo, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Domo service.

Setup Domo as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Domo documentation to set it up as SAML Service Provider.
