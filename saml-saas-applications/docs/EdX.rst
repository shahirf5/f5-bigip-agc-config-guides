...
For example, to configure EdX, select EdX and click ``Add``.
...
To configure EdX provide following inputs:
	- ``Sub Domain`` : Provide the sub domain. For eg - If the SSO URL is https://company.edx.org/auth/complete/tpa-saml/, then the sub domain is company.
...
#. Select the SAML SSO object created for for EdX, and click ``Export Metadata``.
