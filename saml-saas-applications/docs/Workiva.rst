...
For example, to configure Workiva, select Workiva and click ``Add``.
...
To configure Workiva provide following inputs:
	- ``Unique ID`` : Provide the workiva unique id from the SSO URL. For eg - If SSO URL is https://webfilings.appspot.com/auth/saml/sp/metadata/abcd1234, then abcd1234 is the workiva id.
...
#. Select the SAML SSO object created for for Workiva, and click ``Export Metadata``.
