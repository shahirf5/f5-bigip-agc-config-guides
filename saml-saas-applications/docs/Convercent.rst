...
For example, to configure Convercent, select Convercent and click ``Add``.
...
To configure Convercent provide following inputs:
	- ``Sub Domain`` : Provide the sub domain. For eg - If the SSO URL is https://company.convercent.com/Saml20/Saml20, then the sub domain is company.
...
#. Select the SAML SSO object created for for Convercent, and click ``Export Metadata``.
