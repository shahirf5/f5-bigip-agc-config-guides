...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Socialcast:
...
Socialcast Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Socialcast, select Socialcast and click :guilabel:`Add`.
...
To configure Socialcast provide following inputs:
	- :guilabel:`Sub Domain` : If your SSO url is https://company.socialcast.com, sub domain is company
...
#. Select the SAML SSO object created for for Socialcast, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Socialcast service.

Setup Socialcast as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Socialcast documentation to set it up as SAML Service Provider.
