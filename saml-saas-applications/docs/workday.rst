...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Workday:
...
Workday Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Workday, select Workday and click :guilabel:`Add`.
...
To configure Workday provide following inputs:
	- :guilabel:`Tenant Name` : Provide your tenant name from the ACS URL. For eg - If the URL is https://company.workday.com/test/login-saml.htmld, then test is the tenant name.
	- :guilabel:`Domain Name` : Provide the domain name for the workday service.
...
#. Select the SAML SSO object created for for Workday, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Workday service.

Setup Workday as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Workday documentation to set it up as SAML Service Provider.
