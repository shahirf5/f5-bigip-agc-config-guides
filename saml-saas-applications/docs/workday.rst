...
For example, to configure Workday, select Workday and click ``Add``.
...
To configure Workday provide following inputs:
	- ``Tenant Name`` : Provide your tenant name from the ACS URL. For eg - If the URL is https://company.workday.com/test/login-saml.htmld, then test is the tenant name.
	- ``Domain Name`` : Provide the domain name for the workday service.
...
#. Select the SAML SSO object created for for Workday, and click ``Export Metadata``.
