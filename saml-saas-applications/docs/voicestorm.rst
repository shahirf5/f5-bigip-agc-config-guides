...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Voicestorm:
...
Voicestorm Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Voicestorm, select Voicestorm and click :guilabel:`Add`.
...
To configure Voicestorm provide following inputs:
	- :guilabel:`Sub Domain` : Specify sub domain. For example, if your ACS URL is https://acme.voicestorm.com/User/SsoResponse, then enter acme. Entity ID https://acme.voicestorm.com also uses sub domain.
...
#. Select the SAML SSO object created for for Voicestorm, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Voicestorm service.

Setup Voicestorm as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Voicestorm documentation to set it up as SAML Service Provider.
