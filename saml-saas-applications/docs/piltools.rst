...
For example, to configure PILtools, select PILtools and click ``Add``.
...
To configure PILtools provide following inputs:
	- ``Customer ID`` : Specify Customer ID value. Entity ID uses this value. If your ACS URL is https://stg.piltools.com/sso/customerId/, then enter customerId
...
#. Select the SAML SSO object created for for PILtools, and click ``Export Metadata``.
