...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure PILtools:
...
PILtools Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure PILtools, select PILtools and click :guilabel:`Add`.
...
To configure PILtools provide following inputs:
	- :guilabel:`Customer ID` : Specify Customer ID value. Entity ID uses this value. If your ACS URL is https://stg.piltools.com/sso/customerId/, then enter customerId
...
Refer to PILtools documentation to set it up as SAML Service Provider.
