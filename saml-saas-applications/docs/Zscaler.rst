...
For example, to configure Zscaler, select Zscaler and click :guilabel:`Add`.
...
To configure Zscaler provide following inputs:
	- :guilabel:`Zscaler Cloud Domain Name` : Provide the Zscaler cloud name from the SSO URL. For eg - If URL is https://admin.zscloud.net/adminsso.do, then the domain name is zscloud.net only.
...
#. Select the SAML SSO object created for for Zscaler, and click :guilabel:`Export Metadata`.
...
Refer to Zscaler documentation to set it up SAML Service Provider.
