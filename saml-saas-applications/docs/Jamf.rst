...
For example, to configure Jamf, select Jamf and click :guilabel:`Add`.
...
To configure Jamf provide following inputs:
	- :guilabel:`Hostname` : Provide the hostname from the SSO URL. For eg, If the SSO URL is https://abcd.com/saml/SSO, then the hostname is abcd.com.
...
#. Select the SAML SSO object created for for Jamf, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Jamf service.

Setup Jamf as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Jamf documentation to set it up as SAML Service Provider.
