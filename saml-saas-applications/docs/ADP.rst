...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure ADP:
...
ADP Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure ADP, select ADP and click :guilabel:`Add`.
...
To configure ADP provide following inputs:
	- :guilabel:`Person Immutable ID` : Provide the session variable which will map to the personimmutableID attribute. Assertion value include this value.
...
#. Select the SAML SSO object created for for ADP, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the ADP service.

Setup ADP as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to ADP documentation to set it up as SAML Service Provider.
