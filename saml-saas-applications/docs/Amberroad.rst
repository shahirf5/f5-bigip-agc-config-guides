...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Amberroad:
...
Amberroad Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Amberroad, select Amberroad and click :guilabel:`Add`.
...
To configure Amberroad provide following inputs:
	- :guilabel:`Sub Domain` : Provide the sub domain from SSO URL. For eg - If your SSO URL is https://company.amberroad.com/gtm/ACSServlet, then the sub domain is company
...
#. Select the SAML SSO object created for for Amberroad, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Amberroad service.

Setup Amberroad as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Amberroad documentation to set it up as SAML Service Provider.
