=================================================================
SaaS Application Configuration Guide : Kenexa BrassRing(IBM)
=================================================================

BIG-IP as SAML IdP Configuration
--------------------------------
This document describes configuration steps for configuring an AGC SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Kenexa BrassRing(IBM):
...
Kenexa BrassRing(IBM) Configuration in AGC Workflow
----------------------------------------------
...
Kenexa BrassRing(IBM), select
Kenexa BrassRing(IBM) and click on **Add** button
...
To configure Kenexa BrassRing(IBM) provide following inputs:
	- **Sub Domain** : Enter your sub domain. For example, if your ACS URL is https://yourcompany.brassring.com/SSO/SAML/SAML2PageListener/AssertionConsumerService.aspx, enter: yourcompany
...
#. Identify the Saml SSO object created for for Kenexa BrassRing(IBM) and export SAML Metadata.

You can use the exported IdP SAML Metadata to configure the IdP Provider configuration in Kenexa BrassRing(IBM) service.
