======================================================================================
SaaS Application Configuration Guide : Adverity
======================================================================================

BIG-IP as SAML IdP Configuration
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Adverity:
...
Adverity Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Adverity, select Adverity and click :guilabel:`Add`.
...
To configure Adverity provide following inputs:
	- :guilabel:`Company Id` : Provide the company ID from the relay state. For eg, if the relay state is https://company.datatap.adverity.com/login/saml/?idp=abcd, then the company id is abcd.
	- :guilabel:`Sub Domain` : Provide the sub domain. For eg - If the SSO URL is https://company.datatap.adverity.com/complete/saml/, then the sub domain is company.
...
#. Select the SAML SSO object created for for Adverity, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Adverity service.

Setup Adverity as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Adverity documentation to set it up as SAML Service Provider.
