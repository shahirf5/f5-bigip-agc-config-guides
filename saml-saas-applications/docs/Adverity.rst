...
For example, to configure Adverity, select Adverity and click ``Add``.
...
To configure Adverity provide following inputs:
	- ``Company Id`` : Provide the company ID from the relay state. For eg, if the relay state is https://company.datatap.adverity.com/login/saml/?idp=abcd, then the company id is abcd.
	- ``Sub Domain`` : Provide the sub domain. For eg - If the SSO URL is https://company.datatap.adverity.com/complete/saml/, then the sub domain is company.
...
#. Select the SAML SSO object created for for Adverity, and click ``Export Metadata``.
