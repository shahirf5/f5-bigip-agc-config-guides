...
For example, to configure inmotionnow, select inmotionnow and click ``Add``.
...
To configure inmotionnow provide following inputs:
	...
	- ``Sub Domain`` : Provide the sub domain from the SSO URL. For eg if SSO URL is https://company.inmotionnow.com/saml/post, then the sub domain is company
...
#. Select the SAML SSO object created for for inmotionnow, and click ``Export Metadata``.
