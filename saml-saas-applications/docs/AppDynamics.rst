...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure AppDynamics:
...
AppDynamics Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure AppDynamics, select AppDynamics and click :guilabel:`Add`.
...
To configure AppDynamics provide following inputs:
	- :guilabel:`Sub Domain` : Provide the sub domain. For e.g. if the Entity ID URL is https://company.saas.appdynamics.com/controller, the sub domain is company
...
#. Select the SAML SSO object created for for AppDynamics, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the AppDynamics service.

Setup AppDynamics as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to AppDynamics documentation to set it up as SAML Service Provider.
