...
For example, to configure Workfront, select Workfront and click :guilabel:`Add`.
...
To configure Workfront provide following inputs:
	- :guilabel:`Company ID` : Provide the company ID. For eg - If the SSO URL is https://testcompany.attask-ondemand.com/attask/saml2consumer.cmd, then company Id is testcompany.
...
#. Select the SAML SSO object created for for Workfront, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Workfront service.

Setup Workfront as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Workfront documentation to set it up as SAML Service Provider.
