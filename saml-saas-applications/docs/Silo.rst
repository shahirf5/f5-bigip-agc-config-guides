...
--------------------------------
This document describes configuration steps for configuring an AGC SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Silo Authentic8:
...
Silo Authentic8 Configuration in AGC Workflow
----------------------------------------------
...
Silo Authentic8, select
Silo Authentic8 and click on **Add** button
...
To configure Silo Authentic8 provide following inputs:
	- **Company ID** : Provide your unique company ID from the SSO URL. For eg, from https://getsilo.com/sso/saml/company/login URL, company is the company ID.
...
#. Identify the Saml SSO object created for for Silo Authentic8 and export SAML Metadata.

You can use the exported IdP SAML Metadata to configure the IdP Provider configuration in Silo Authentic8 service.
