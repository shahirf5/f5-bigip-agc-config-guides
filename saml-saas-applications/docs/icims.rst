...
For example, to configure ICIMS, select ICIMS and click :guilabel:`Add`.
...
To configure ICIMS provide following inputs:
	- :guilabel:`Sub Domain` : Provide the Sub Domain. For e.g. if the ACS URL is https://company.icims.com/icims2/servlet/icims2?module=Root&action=samlAssertionConsumerService, the Sub Domain is company.
...
#. Select the SAML SSO object created for for ICIMS, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the ICIMS service.

Setup ICIMS as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to ICIMS documentation to set it up as SAML Service Provider.
