...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Signmesh:
...
Signmesh Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Signmesh, select Signmesh and click :guilabel:`Add`.
...
To configure Signmesh provide following inputs:
	- :guilabel:`Sub Domain` : Provide the sub domain. For eg - If the SSO URI is https://company.signmesh.com/public_html/simplesaml/module.php/saml/sp/metadata.php/default-sp, then the sub domain is company.
...
Refer to Signmesh documentation to set it up as SAML Service Provider.
