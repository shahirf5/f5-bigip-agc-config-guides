...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Vivi:
...
Vivi Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Vivi, select Vivi and click :guilabel:`Add`.
...
To configure Vivi provide following inputs:
	- :guilabel:`Unique Organization ID` : Provide your unique organization ID. For eg - If Entity ID is https://api.vivi.io/api/v1/users/saml_metadata/abcd-1234, then abcd-1234 is the unique id.
...
#. Select the SAML SSO object created for for Vivi, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Vivi service.

Setup Vivi as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Vivi documentation to set it up as SAML Service Provider.
