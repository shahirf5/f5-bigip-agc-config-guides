...
For example, to configure Greenhouse, select Greenhouse and click ``Add``.
...
To configure Greenhouse provide following inputs:
	- ``Sub Domain`` : Provide the sub domain. For eg if the ACS URL is https://company.greenhouse.io/users/saml/consume, the sub domain is company.
...
#. Select the SAML SSO object created for for Greenhouse, and click ``Export Metadata``.
