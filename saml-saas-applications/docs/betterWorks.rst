...
For example, to configure betterWorks, select betterWorks and click ``Add``.
...
To configure betterWorks provide following inputs:
	- ``SAML Token`` : Unique ID assigned by BetterWorks
...
#. Select the SAML SSO object created for for betterWorks, and click ``Export Metadata``.
