...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Aerohive IDManager:
...
Aerohive IDManager Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Aerohive IDManager, select Aerohive IDManager and click :guilabel:`Add`.
...
To configure Aerohive IDManager provide following inputs:
	- :guilabel:`Account ID` : Provide the account ID from the callback URL. For eg, if the callback URL is https://idmanager.aerohive.com/idmanager/register/saml/callback/123456, then the account id is 123456
...
#. Select the SAML SSO object created for for Aerohive IDManager, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Aerohive IDManager service.

Setup Aerohive IDManager as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Aerohive IDManager documentation to set it up as SAML Service Provider.
