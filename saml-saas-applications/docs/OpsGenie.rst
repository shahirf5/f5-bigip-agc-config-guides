...
--------------------------------
This document describes configuration steps for configuring an AGC SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure OpsGenie:
...
OpsGenie Configuration in AGC Workflow
----------------------------------------------
...
OpsGenie, select
OpsGenie and click on **Add** button
...
To configure OpsGenie provide following inputs:
	- **Unique Account ID** : Provide your unique account ID. For eg - If the SSO URL is https://app.opsgenie.com/auth/saml?id=company, then the unique id is company
...
#. Identify the Saml SSO object created for for OpsGenie and export SAML Metadata.

You can use the exported IdP SAML Metadata to configure the IdP Provider configuration in OpsGenie service.
