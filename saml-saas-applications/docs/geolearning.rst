...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure GeoLearning:
...
GeoLearning Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure GeoLearning, select GeoLearning and click :guilabel:`Add`.
...
To configure GeoLearning provide following inputs:
	- :guilabel:`Customer ID` : Specify Customer Id. For example in entityId https://gm1.geolearning.com/geonext/customerId/saml.geo, customerId is the Cutomer ID.
...
#. Select the SAML SSO object created for for GeoLearning, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the GeoLearning service.

Setup GeoLearning as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to GeoLearning documentation to set it up as SAML Service Provider.
