...
For example, to configure Snaplogic, select Snaplogic and click ``Add``.
...
To configure Snaplogic provide following inputs:
	- ``Entity ID`` : Provide the entity ID from the SAML metadata. It looks like - https://company.com/api/1/rest/admin/sso/login?org_path=company
...
#. Select the SAML SSO object created for for Snaplogic, and click ``Export Metadata``.
