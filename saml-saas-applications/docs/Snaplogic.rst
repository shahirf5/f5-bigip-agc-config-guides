...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Snaplogic:
...
Snaplogic Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Snaplogic, select Snaplogic and click :guilabel:`Add`.
...
To configure Snaplogic provide following inputs:
	- :guilabel:`Entity ID` : Provide the entity ID from the SAML metadata. It looks like - https://company.com/api/1/rest/admin/sso/login?org_path=company
...
Refer to Snaplogic documentation to set it up as SAML Service Provider.
