...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Benefit Solver:
...
Benefit Solver Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Benefit Solver, select Benefit Solver and click :guilabel:`Add`.
...
To configure Benefit Solver provide following inputs:
	- :guilabel:`Sub Domain` : Provide the sub domain. For eg - If the SSO URL is https://$subDomain$.benefitsolver.com/saml20/, then the sub domain is company.
...
Refer to Benefit Solver documentation to set it up as SAML Service Provider.
