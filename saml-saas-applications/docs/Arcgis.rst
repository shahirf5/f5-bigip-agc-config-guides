...
--------------------------------
This document describes the configuration for a SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure Arcgis:
...
Arcgis Configuration in Guided Configuration Workflow
---------------------------------------------------------------------------------------------------------------------------
...
For example, to configure Arcgis, select Arcgis and click :guilabel:`Add`.
...
To configure Arcgis provide following inputs:
	- :guilabel:`Sub Domain` : If your SSO URL is https://company.maps.arcgis.com/sharing/rest/oauth2/saml/signin, then sub domain is company.
...
#. Select the SAML SSO object created for for Arcgis, and click :guilabel:`Export Metadata`.
#. Use the exported IdP SAML Metadata to configure the IdP Provider configuration in the Arcgis service.

Setup Arcgis as SAML Service Provider
------------------------------------------------------------------------------------------------------------

Refer to Arcgis documentation to set it up as SAML Service Provider.
