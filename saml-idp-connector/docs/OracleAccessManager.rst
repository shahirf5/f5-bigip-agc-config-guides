...
-------------------------------
This document describes the configuration for an external IDP Connector using an IDP Connector template in the Guided Configuration SAML Service Provider workflow. Follow the steps below to configure Oracle Access Manager:
...
#. Select the Identity Provider from the provided set. You can search for Oracle Access Manager in the Search bar. If a template for your application is not available, you can use the options :guilabel:`Metadata` or :guilabel:`Custom`, and configure the Identity Providers accordingly.

Select Oracle Access Manager and click :guilabel:`Add`. Specify a name for the connector.
...
Setup Oracle Access Manager as SAML Identity Provider
-----------------------------------------------------------------

Refer to Oracle Access Manager documentation to set it up as SAML Identity Provider.
