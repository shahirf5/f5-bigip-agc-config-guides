...
#. Select the Identity Provider from the provided set. You can search for BIG-IP IdP in the Search bar. If a template for your application is not available, you can use the options :guilabel:`Metadata` or :guilabel:`Custom`, and configure the Identity Providers accordingly.

Select BIG-IP IdP and click :guilabel:`Add`. Specify a name for the connector.

IDP Connector Specific Properties
---------------------------------

To configure BIG-IP IdP provide following inputs:
	- :guilabel:`IdP Hostname` : Provide the hostname of your Identity Provider.
